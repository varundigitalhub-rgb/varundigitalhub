...
const TEMPLATE_NAME = "Local Shopkeeper Website";
const BASE_PATH = "/templates/shopkeeper/shopkeeper-local";

const navLinks = [
  { href: BASE_PATH, label: "Home" },
  { href: `${BASE_PATH}/about`, label: "About" },
  { href: `${BASE_PATH}/products`, label: "Products" },
  { href: `${BASE_PATH}/offers`, label: "Offers" },
  { href: `${BASE_PATH}/contact`, label: "Contact" },
];

export default function ShopkeeperLocalLayout({
  children,
}: {
  children: ReactNode;
}) {
      ...
      <header className="sticky top-0 z-40 border-b border-emerald-100 bg-white/95 backdrop-blur">
        <div className="container mx-auto flex items-center justify-between px-6 py-3">
          <Link href={BASE_PATH} className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-lg bg-emerald-50 flex items-center justify-center overflow-hidden">
              {brandLogo ? (
                <Image
                  src={brandLogo}
                  alt={brandName}
      ...
      <footer className="border-t border-emerald-100 bg-emerald-900 text-white">
        <div className="container mx-auto flex flex-col gap-3 px-6 py-4 md:flex-row md:items-center md:justify-between">
          <p className="text-xs md:text-sm text-emerald-50">
            Like this Local Business website? Show products, timings, and
            contact buttons for your own shop.
          </p>
          <div className="flex flex-col gap-2 sm:flex-row">
            <Link
              href="/contact"
              className="inline-flex justify-center rounded-full bg-white px-5 py-2 text-xs md:text-sm font-semibold text-emerald-900 shadow-sm hover:bg-emerald-50"
            >
              Get This Website
            </Link>
            <a
