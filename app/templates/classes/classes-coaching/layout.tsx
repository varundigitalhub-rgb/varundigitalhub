...
export interface ClassesCoachingLayoutProps {
  children: ReactNode;
  brandName?: string;
  brandLogo?: string | null;
}

const TEMPLATE_NAME = "Classes – Coaching Institute";
...
}: ClassesCoachingLayoutProps) {
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50">
      <header className="border-b border-slate-800 bg-slate-950/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-sky-400 text-xs font-semibold text-slate-950">
              {brandName.charAt(0)}
            </div>
            <div>
          ...
          <nav className="flex flex-wrap items-center gap-3 text-xs">
            ...
            <Link href={`${BASE_PATH}/about`} className="hover:text-sky-200">
              About
            </Link>
            <Link href={`${BASE_PATH}/courses`} className="hover:text-sky-200">
              Courses
            </Link>
            <Link href={`${BASE_PATH}/faculty`} className="hover:text-sky-200">
              Faculty
            </Link>
          ...
          <div className="flex flex-wrap items-center gap-2 text-[11px]">
            ...
            <Link
              ...
              className="rounded-full border border-sky-300 px-3 py-2 font-semibold text-sky-100 hover:bg-sky-400/10"
            >
              Chat on WhatsApp
            </Link>
          </div>
